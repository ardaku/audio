...
extern "C" {
    ...
    fn ope_encoder_write(enc: *mut c_void, pcm: *const i16, samples_per_channel: i32) -> i32;
    fn ope_encoder_get_page(enc: *mut c_void, page: *mut *mut u8, len: *mut i32, flush: i32)
        -> i32;
...
impl OpusEncoder {
    ...
    pub fn new(hz: i32, channels: i32, voip: bool) -> Result<*mut Self, i32> {
        ...
        unsafe {
            let opus_encoder =
                opus_encoder_create(hz, channels, if voip { 2048 } else { 2049 }, e.as_mut_ptr());
            let e = e.assume_init();
            if e != 0 {
                Err(e)
            } else {
    ...
    pub fn encode(self2: *mut Self, raw: &[i16; 1920 * 2], outbuf: &mut [u8]) -> usize {
        unsafe {
            let ret = opus_encode(
                self2,        // encoder state
                raw.as_ptr(), // input
...
impl Comments {
    ...
    pub fn add(&self, tag: &str, val: &str) -> i32 {
        ...
        unsafe { ope_comments_add(self.0, tag.as_ptr() as *const _, val.as_ptr() as *const _) }
    }
}

impl Drop for Comments {
    fn drop(&mut self) {
        unsafe { ope_comments_destroy(self.0) }
    }
}
...
impl Encoder {
    pub fn new(comments: &Comments, rate: i32, channels: i32, family: i32) -> Result<Self, i32> {
        let mut error = std::mem::MaybeUninit::uninit();
        let enc = unsafe {
            ope_encoder_create_pull(comments.0, rate, channels, family, error.as_mut_ptr())
        };

        if enc.is_null() {
            unsafe { Err(error.assume_init()) }
        } else {
            Ok(Self(enc))
        }
    }
    ...
    pub fn get(&self) -> Option<&[u8]> {
        ...
        unsafe {
            let page_available =
                ope_encoder_get_page(self.0, page_data.as_mut_ptr(), page_size.as_mut_ptr(), 0);
            if page_available != 0 {
                Some(std::slice::from_raw_parts(
                    page_data.assume_init(),
                    page_size.assume_init() as usize,
                ))
            } else {
                None
            }
